...
% TODO:
\subsection{Bosques aleatorios(terminar)}
...
Decision tree
% para funciones con resultados discretos
% se pueden representar como un if/then
% se puede entender las decisiones tomadas
% cada nodo es una prueba a un atributo y cada rama de este nodo es un valor diferente de ese atributo
% la principal decision es cual atributo prueba en cada nodo
% se utiliza en cada nivel el atributo que mas \textit{ganancia de información} tenga
% entropia: caracteriza la pureza de un conjunto $E(S) @= - p_{\oplus}log_{2}(p_{\oplus}) - p_{\ominus}log_{2}(p_{\ominus})$ probabilidad del caso negativo y del positivo (oplus y ominus)
% si el conjunto tiene c categorías entonces la entropia es: $E(S) @= \sum_{i=1}^c -p_{i}log_{2}(p_{i})$
% ganancia de informacion del conjunto S con el atributo A: $G(S, A) @= E(S) - \sum_{v \in Values(A)} \frac{S_{v}}{S}E(S_{v})$ donde Values(A) son todos los posibles valores que puede tomar el atributo A y S_{v} es el subconjunto de S donde el atributo A toma el valor v
% el algoritmo empieza por la raiz seleccionando el atributo que mas ganancia de informacion tenga, crea tantas ramas como valores difrentes tenga ese atributo y baja al siguiente nivel para repetir este proceso hasta que se acaben todos los atributos

Selector de características, esto se hace promediando la importancia de las características en cada árbol.

% por que se escogió este
% definición, bagging, decision tree

% explicación del funcionamiento del algoritmo y como mejora los resultados en este problema[ver tesis de RNN en IDS]

% TODO: esta bibliografía para la parte de features selection
