...
% \cite{ludwig2019applying} : NSL-KDD, ensemble deep neural network(sistemas compuestos por varias redes neuronales)(autoencoder, deep belief neural network, deep neural network, extreme learning machine rbm), clasificación binaria y multiple, no especifican si los resultados fueron con el kdd.test, nada de reducción de dimensiones

% \cite{samrin2018hybrid} : KDD99, k-means y n redes neuronales cada una entrenada en dar una respuesta binaria con un resultado diferente del k-means para luego unificarlas(ensemble), nada de reducción de dimensiones

% \cite{divyasree2018network} : KDD99, clasificación multiple, utilizan solo 21 features(básicos y de contenido, ignoran los de tráfico), para la clasificación de cada ataque diferente seleccionan 10 features de los 21 utilizando el test de chi cuadrado, CVM

% \cite{almseidin2017evaluation} : KKD99, varios clasificadores(j48, random forest, random tree, decision table, mlp, naive bayes, bayes network) mejor clasificador random forest(93.77 acc), 41 features

% \cite{abualkibash2019machine} : NSL-KDD, ponen los resultados con KDDTrain(70-30) y KDDTest, utilizan varios algoritmos(decision tree, random forest, naive bayes, svm, pnn, gradient boosted trees, k-nn), ninguno ANN, mejores resultados con el train random forest y decision trees, mejores resultados con el test naive bayes, 41 features

% \cite{goyal2008ga} : KDD99, algoritmos genéticos, 41 features

% \cite{ahmadi2009intrusion} : KDD99, otra variante de algoritmos genéticos, 41 features

% \cite{kabanda2019bayesian} : KDD99, utilizan varias arquitecturas(svm, ann, knn, naive bayes, decision trees), hacen un examen de los features pero no llegan a nada, 41 features

% \cite{sammany2007artificial} : KDD99, ANN, clasificación multiple, 35 features, no dicen como los separaron

% \cite{hasan2016feature} : NSL-KDD, features selection with random forest, buenísimo trabajo porque lo explican todo al detalle y muchas gráficas bien claras

% \cite{anani2018recurrent} : KDD99, RNN, utilizan random forest y PCA para reducción de features mostrando RF con mejores resultados, el trabajo se centra en ver el tiempo de entrenamiento de una rnn con este dataset
